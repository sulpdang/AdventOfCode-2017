...
object Main extends Day(24) {
  ...
  def solve(input:Input) = {
    implicit def order:Ordering[Result] = new Ordering[Result] {
      def compare(x:Result, y:Result) = x.m - y.m
    }
  ...
  def solve2(input:Input) = {
    implicit def order:Ordering[Result] = new Ordering[Result] {
      def compare(x:Result, y:Result) = if(x.l != y.l) x.l - y.l else x.m - y.m
    }
    traverseChild(0, input.toSet)
  }
